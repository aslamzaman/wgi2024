...
const Delivery = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            ...
            try {
                const customers = await GetRemoteData('customer');
                ...
                const result = data.map(order => {
                    const matchCustomer = customers.find(customer => customer._id === order.orderId.customerId);
                    return { ...order, matchCustomer };
                })
    ...
    const printHandler = (id) => {
        ...
        setTimeout(() => {
            ...
            doc.setFont("Poppins-Regular", "normal");
            doc.text(`${invoice.matchCustomer.address}`, 20, 90, null, null, "left");
            doc.text(`${invoice.matchCustomer.contact}`, 20, 95, null, null, "left");
            doc.setFontSize(7);
            ...
            let y = 112;
            let subTotal = 0;
            let items = invoice.orderId.items;
                            ...
                            {deliverys.length ? (
                                deliverys.map(delivery => (
                                    <tr className="border-b border-gray-200 hover:bg-gray-100" key={delivery._id}>
                                        ...
                                        <td className="text-center py-2 px-4">{delivery.matchCustomer.name}</td>
                                        ...
                                        <td className="text-center py-2 px-4">{delivery.advance}</td>
