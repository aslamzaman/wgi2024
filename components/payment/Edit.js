...
const Edit = ({ message, id, data }) => {        
    ...
    const showEditForm =  () => {
        setShow(true);
         const { customerObject, dt, cashtypeObject, bank, taka } = data.find(payment => payment._id === id) || { customerObject: '', dt: '', cashtypeObject: '', bank: '', taka: '' };
         setCustomerObject(customerObject);
         setDt(dt);
         setCashtypeObject(cashtypeObject);
         setBank(bank);
         setTaka(taka);             
    };
                        ...
                        <div className="px-6 pb-6 text-black">
                            <form onSubmit={saveHandler} >
                                <div className="grid grid-cols-1 gap-4 my-4">
                                    <TextEn Title="Customerobject" Id="customerObject" Change={e => setCustomerObject(e.target.value)} Value={customerObject} Chr={50} />
                                    <TextEn Title="Dt" Id="dt" Change={e => setDt(e.target.value)} Value={dt} Chr={50} />
                                    <TextEn Title="Cashtypeobject" Id="cashtypeObject" Change={e => setCashtypeObject(e.target.value)} Value={cashtypeObject} Chr={50} />
