...
const Add = ({ message }) => {
    ...
    const showAddForm = async () => {
        ...
        resetVariables();
        try {
            const responseBorrower = await GetRemoteData('borrower');
            console.log(responseBorrower);
            const sortResult = responseBorrower.sort((a, b) => a.name.toUpperCase() < b.name.toUpperCase() ? -1 : +1);
            console.log("Sort", sortResult);
            setBorrowers(sortResult);
        } catch (error) {
    ...
    const saveHandler = async (e) => {
        ...
        try {
            ...
            const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/loan`;
            ...
            if (response.ok) {
                message(`Loan is created at ${new Date().toISOString()}`);
            } else {
                throw new Error("Failed to create loan");
            }
        } catch (error) {
            console.error("Error saving loan data:", error);
            message("Error saving loan data.");
        } finally {
            setShow(false);
        }
    }
