...
const Delete = ({ message, id, data }) => {
    ...
    const showDeleteForm = () => {
        setShow(true);
        try {
           const { name } = data.find(item => item._id === id) || { name: "" };
           setName(name);
    ...
    const deleteYesClick = async () => {
        try {
            const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/item/${id}`;
    ...
    return (
        <>
            ...
            <button onClick={showDeleteForm} title="Delete" className="px-1 py-1 hover:bg-red-300 rounded-md transition duration-500">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6 stroke-black hover:stroke-blue-800 transition duration-500">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
        </>
