...
const Delete = ({ message, id, data }) => {
    ...
    const showDeleteForm = () => {
        setShow(true);
        const { name } = data.find(borrower => borrower._id === id) || { name: "" };
        setName(name); 
    }
    ...
    const softDeleteHandler = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/borrower/${id}`, {
                method: "PATCH",
...
/*
    const hardDeleteHandler = async () => {
        try {
            const apiUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/api/borrower/${id}`;
