...
const Edit = ({ message, id, data }) => {        
    ...
    const showEditForm =  () => {
        setShow(true);
         const { name, address, contact, isDeleted } = data.find(customer => customer._id === id) || { name: '', address: '', contact: '', isDeleted: '' };
         setName(name);
         setAddress(address);
         setContact(contact);
                        ...
                        <div className="px-6 pb-6 text-black">
                            <form onSubmit={saveHandler} >
                                <div className="grid grid-cols-1 gap-4 my-4">
                                    ...
                                    <TextEn Title="Isdeleted" Id="isDeleted" Change={e => setIsDeleted(e.target.value)} Value={isDeleted} Chr={50} />                                        
                                </div>
