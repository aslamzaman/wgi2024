...
const Delete = ({ message, id, data }) => {
    ...
    const showDeleteForm = () => {
        setShow(true);
        const { borrowerId } = data.find(loanpayment => loanpayment._id === id) || { borrowerId: "" };
...
/*
    const softDeleteHandler = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/loanpayment/${id}`, {
                method: "PATCH",
    ...
    const hardDeleteHandler = async () => {
        try {
            ...
            if (response.ok) {
                ...
                throw new Error("Failed to delete loanpayment");
            }         
        } catch (error) {
            console.log(error);
                    ...
                    <div className="w-11/12 md:w-1/2 mx-auto mb-10 bg-white border-2 border-gray-300 rounded-md shadow-md duration-300">
                        ...
                        <div className="p-4 lg:p-6 flex flex-col space-y-4">
                            <div className="w-full">    
                                ...
                                <h1 className="text-center text-gray-600 font-bold">{borrowerId}</h1>
                            </div>
